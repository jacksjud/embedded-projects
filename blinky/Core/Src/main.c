...
void blink(){
	// If not pressed (when pressed, input will read 0)
		  if(B1_Button_Port->IDR & (0b1 << B1_Button_Pin)){
			  // Resets output pin
			  LD2_Green_LED_Port->BSRR |= (0b1 << (LD2_Green_LED_Pin + 16));
		  }
		  else {
			  // Sets output pin
			  LD2_Green_LED_Port->BSRR |= (0b1 << LD2_Green_LED_Pin);
		  }
}
/* USER CODE END 0 */
...
//	  LD2_Green_LED_Port->BSRR |= (0b1 << LD2_Green_LED_Pin);
	  /*
	   * Input should read 1 when button not pressed (since button is set to be pull-up resistor)
